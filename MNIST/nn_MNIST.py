...
def train_model(args):
    ...
    def reg_f(params, l2_reg_params, l1_reg_params=None):
        r = 0.5 * ((params[0].T ** 2) * torch.exp(l2_reg_params.unsqueeze(1) * ones_dxc.T)).mean()
        if l1_reg_params is not None:
            r += (params[0].abs() * torch.exp(l1_reg_params.unsqueeze(1) * ones_dxc)).mean()
        return r
    def out_f(x, params):
        out = F.linear(x,params[0]).requires_grad_(True) # x @ params[0]
    ...
    ones_dxc = torch.ones(n_classes, n_features) # (n_features, n_classes) 
    ...
    w = torch.zeros(n_classes, n_features).requires_grad_(True) # (n_features, n_classes)
    parameters = [w]
