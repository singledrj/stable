...
def parse_args():
    ...
    parser.add_argument('--inner_lr', type=float, default=3e-2) # beta
    parser.add_argument('--inner_mu', type=float, default=0.0)
    parser.add_argument('--outer_lr', type=float, default=5e-6) # alpha
...
def train_model(args):
    ...
    # load dataset
    val_size_ratio = 0.5
    file = 'sonar/sonar.all-data'
    data = []
    label = []
    f =  open(file,'r')
    for line in f:
        datum = line.strip().split(',')
        data.append(datum[0:-1])
        if datum[-1] == 'R':
            label.append(0)
        elif datum[-1] == 'M':
            label.append(1)
